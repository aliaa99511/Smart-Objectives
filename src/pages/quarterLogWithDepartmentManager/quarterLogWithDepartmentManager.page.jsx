...
import styles from "./quarterLogWithDepartmentManager.module.css";
...
import { useGetQuartersLogWithManagerQuery } from "../../appState/apis/managerApprovalsSoApiSlice";
...
const QuarterLogWithDepartmentManager = () => {
  ...
  return (
    <>
          ...
          <Widget>
            <div className={styles.details}>
              <div className="user">
                <UserInfo
                  userData={{
                    name: quartersLogData?.employeeDetails?.name || "",
                    img: quartersLogData?.employeeDetails?.img || "",
                    jobTitle: quartersLogData?.employeeDetails?.jobTitle || "",
                    badge: quartersLogData?.employeeDetails?.badge || "",
                    notificationCount:
                      quartersLogData?.employeeDetails?.pendingRequests || null,
                  }}
                  imgSize="imgLG"
                  nameSize="nameSM"
                  isLoading={isLoading}
      ...
      {isLoading || isFetching ? (
        <MainLoader height={"calc(100vh - 275px)"} />
      ) : isError ? (
        <TryAgain
                ...
                <Grid key={q} size={{ xs: 12, md: 6 }}>
                  ...
                  >
                    <QuarterCard
                      ...
                      year={selectedYear}
                      quarter={q}
                      quarterData={quartersLogData?.data?.[q]}
                    />
                  </Widget>
                </Grid>
              ))}
          ...
          {[1, 2, 3, 4].map((quarter) => (
            <Widget key={quarter} margin="0 0 10px 0">
              ...
              <Table
                ...
                rows={getTableRows(
                  quartersLogData?.data?.[quarter]?.objectives
                )}
...
export default QuarterLogWithDepartmentManager;
